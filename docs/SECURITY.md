...
本工具涉及本地电脑、SSH 加密连接、Delta 项目空间/计算节点，以及可选的 Tailscale 或 Cloudflare 网络。Tailscale 模式只在 tailnet 内暴露服务；Cloudflare 公网模式意味着请求正文和响应会经过第三方网络。任何方式都应符合 PI、NCSA 和数据治理要求。

## 凭据处理

- NCSA 密码与 Duo 响应只由 OpenSSH 客户端读取，Python 程序无法获得它们。
- 共享 API key 用 Python `secrets` 生成，通过 SSH stdin 发送；远端保存在部署目录下的 `secrets/api_key`，权限 `0600`。默认拓扑中的 Worker 只监听回环地址；拆分拓扑需要通过Delta内部计算网络互联，并使用另一枚随机内部Bearer key保护Worker端点。两种拓扑都只通过统一 Gateway 提供网络入口。
...
- Tailscale auth key 从 `TAILSCALE_AUTHKEY` 读取。批量作业必须使用 Reusable + Ephemeral + Pre-approved key；部署器通过仅所有者可读的临时文件注册节点，随后立即删除该文件。auth key 不等同于模型 API key。
- 本地状态位于 `~/.delta-llm/deployments`，包含 API key。共享电脑上应确认主目录权限，并在实验完成后删除不再需要的状态文件。

## 上线前清单

1. 获得项目 PI/NCSA 对外部网络或 Gateway 的批准。
2. Tailscale 模式只使用 tailnet 私有地址；不要启用 Funnel。Cloudflare 公网模式使用 HTTPS。
...
- Tailscale 通常会尝试点对点连接；若网络环境不允许，会回退到 DERP 中继，因此不能保证每次都比 Cloudflare 延迟更低。
