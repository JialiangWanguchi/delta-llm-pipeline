...
默认固定使用经过 Delta CUDA 12.8 驱动验证的 vLLM 0.10.2 wheel 和 Transformers 4.55.2。不要只修改 `vllm_version`：若要升级，还必须在 `config.toml` 同时填写兼容的 `transformers_version`，以及确实存在且与 Delta 驱动兼容的 `vllm_wheel_url`。
